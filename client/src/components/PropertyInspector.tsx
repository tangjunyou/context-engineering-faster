...
export default function PropertyInspector() {
  ...
  const handleContentChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    const foundVars =
      newContent.match(/\{\{([^}]+)\}\}/g)?.map(v => v.slice(2, -2)) || [];
  ...
  const insertVariable = (v: Variable) => {
    ...
    if (textarea) {
      ...
      const newText =
        text.substring(0, start) + `{{${v.name}}}` + text.substring(end);

      updateNodeData(selectedNode.id, {
        content: newText,
        variables: Array.from(
          new Set([...(selectedNode.data.variables || []), v.id])
        ),
      });
    }
  };
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-2">
            <div className="flex justify-between items-center">
              ...
              <span className="text-[10px] text-muted-foreground">
                {t("propertyInspector.supportsVar")}
              </span>
            </div>
            <Textarea
          ...
          <div className="grid gap-2">
            <Label className="text-xs text-muted-foreground">
              {t("propertyInspector.insertVariable")}
            </Label>
