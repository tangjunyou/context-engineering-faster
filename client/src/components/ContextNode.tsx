...
export default function ContextNode({
  ...
  const colorClass =
    typeColors[data.type as keyof typeof typeColors] || "border-border bg-card";
    ...
    >
      ...
      <div className="p-3">
        <div className="flex items-center gap-2 mb-2">
          <div
            className={cn("p-1.5 rounded-md bg-background/50 backdrop-blur-sm")}
          >
            <Icon size={16} />
          </div>
        ...
        {data.variables && data.variables.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-1">
            {data.variables.map((v: string) => (
              <span
                key={v}
                className="text-[10px] px-1.5 py-0.5 rounded-full bg-background/50 border border-border/50 opacity-80"
              >
                {v}
              </span>
            ))}
