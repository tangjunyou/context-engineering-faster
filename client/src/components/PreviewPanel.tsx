...
export default function PreviewPanel() {
  ...
  const generatePreview = () => {
    ...
    try {
      ...
      setCost((tokens.length / 1000000) * 5.00);
    } catch (e) {
  ...
  return (
    <div className="h-full flex flex-col bg-card border-l border-border">
      <div className="p-4 border-b border-border flex items-center justify-between">
        <h2 className="font-mono font-bold text-sm uppercase tracking-wider">{t('preview.title')}</h2>
      ...
      <div className="p-4 border-b border-border bg-muted/30">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <div className="text-[10px] uppercase text-muted-foreground font-bold tracking-wider">{t('preview.tokens')}</div>
            <div className="text-2xl font-mono font-bold text-primary">{tokenCount}</div>
          </div>
          <div>
            <div className="text-[10px] uppercase text-muted-foreground font-bold tracking-wider">{t('preview.estCost')}</div>
            <div className="text-2xl font-mono font-bold text-green-500">${cost.toFixed(5)}</div>
          </div>
        </div>
      </div>
      ...
      <Tabs defaultValue="preview" className="flex-1 flex flex-col">
        <div className="px-4 pt-2">
          <TabsList className="w-full grid grid-cols-2">
            <TabsTrigger value="preview">{t('preview.contextPreview')}</TabsTrigger>
            <TabsTrigger value="json">{t('preview.jsonStructure')}</TabsTrigger>
          </TabsList>
        </div>
        
        <TabsContent value="preview" className="flex-1 p-0 m-0 relative overflow-hidden">
