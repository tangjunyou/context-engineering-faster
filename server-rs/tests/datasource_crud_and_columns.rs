...
#[tokio::test]
async fn local_sqlite_datasource_works_with_relative_data_dir() {
    std::env::set_var("DATA_KEY", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=");
    ...
    let app = server_rs::build_app_with_data_dir(static_dir, data_dir.clone());
    ...
    let response = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/api/datasources/local/sqlite")
                .header("content-type", "application/json")
                .body(Body::from(body))
                .unwrap(),
        )
    ...
    let bytes = response.into_body().collect().await.unwrap().to_bytes();
    let json: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
    assert!(json["tables"].as_array().is_some());
